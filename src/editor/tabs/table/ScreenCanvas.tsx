...
import type { GameDef, Id, LayoutStyle, ScreenElement, ScreenLayout } from '../../../shared/types';
...
import {
  ...
  indexElements, layoutStyleCss, pathToEl, pctToPx, pruneNested, variantElements,
  withDescendants, zoneSampleCount,
  type AspectPreset, type PlainRect, type VariantKey,
} from './screenModel';
...
export function ScreenCanvas({
  ...
  // Two quick non-drag taps on one element = focus it (covers mouse
  // double-click AND touch double-tap, which never synthesizes dblclick here
  // because pointerdown is prevented).
  ...
  const onCanvasUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    g.lastPinch = null;
    if (g.pointers.size === 0 && !g.moved) onSelect([]);
  };
  ...
  const renderElement = (el: ScreenElement, parentAbs: PlainRect, ghost: boolean): JSX.Element | null => {
    ...
    // Editor-only state preview: merged style + (optional) rect override.
    ...
    return (
      ...
      >
        <ElementBody def={def} el={el} abs={abs} screenH={screenH} screenW={unitW} style={style} />
  ...
  const backdrop = focusEl ? (() => {
    ...
    return (
      <div
        ...
        aria-hidden="true"
      >
        <ElementBody def={def} el={focusEl} abs={ROOT_RECT} screenH={screenH} screenW={unitW} style={bStyle} />
      </div>
    );
  })() : null;
  ...
  return (
    <div className="tt-canvas-wrap">
      ...
      >
        <div
          className="tt-stage"
          style={{ width: SCREEN_W, height: screenH, transform: `translate(${view.x}px, ${view.y}px) scale(${view.z})` }}
        >
...
function ElementBody({ def, el, abs, screenH, screenW, style }: {
  def: GameDef;
  el: ScreenElement;
  abs: PlainRect;
  screenH: number;
  ...
  switch (el.kind) {
    case 'zone': return <ZoneBody def={def} el={el} abs={abs} screenH={screenH} screenW={screenW} />;
    case 'text': return <TextBody def={def} el={el} screenW={screenW} />;
    case 'varText': return <VarTextBody def={def} el={el} screenW={screenW} />;
...
function ZoneBody({ def, el, abs, screenH, screenW }: {
  def: GameDef;
  el: ZoneEl;
  abs: PlainRect;
  screenH: number;
  ...
  return (
    <span className="tt-el-body tt-zone-body">
      <span className="tt-item-label">
        ...
        {el.showCount && <span className="tt-count-tag">×{zoneSampleCount(def, zone.id)}</span>}
      </span>
      <SlotPreview zone={zone} el={el} abs={abs} screenH={screenH} screenW={screenW} />
    </span>
  );
}

/** Simulated card slots from the zone's REAL layout settings (runner math). */
function SlotPreview({ zone, el, abs, screenH, screenW }: {
  ...
  if (el.display === 'piles') {
    // Piles: one slot per card def, layered, × N + (optional) corner badge.
      ...
      >
        {Array.from({ length: n }, (_, i) => (
          <span className="tt-slot tt-slot-pile" key={i} style={{ width: cardW, height: cardH }}>
            <span className="tt-slot-count">× {10 - (i % 3)}</span>
            {el.pileBadgeField != null && <span className="tt-slot-badge">{3 + (i % 5)}</span>}
          </span>
        ))}
      </span>
    );
    ...
    body = (
      <span className="tt-slots tt-slots-stack">
        ...
        {slot(0, { marginLeft: -cardW + 4, marginTop: -4 }, stacked ? '×2' : undefined)}
      </span>
    );
    ...
    body = (
      <span
        ...
        style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, max-content)`, gap: gapPx, justifyContent: 'center' }}
      >
        {Array.from({ length: n }, (_, i) => slot(i, undefined, stacked && i === 0 ? '×2' : undefined))}
      </span>
    );
    ...
    const n = Math.max(1, Math.min(8, Math.floor((innerW - cardW) / step) + 1));
    ...
    body = (
      <span className="tt-slots tt-slots-fan" style={{ display: 'flex' }}>
        {Array.from({ length: n }, (_, i) => {
          ...
          }, stacked && i === Math.floor(mid) ? '×2' : undefined);
        })}
      </span>
    );
  } else {
    const n = fitCount(innerW, cardW, gapPx, 8);
    body = (
...
function TextBody({ def, el, screenW }: { def: GameDef; el: TextEl; screenW: number }) {
  // Dynamic text previews its segments: strings verbatim, expressions as
  // readable ⟨value⟩ chips (the runner substitutes live values).
...
function VarTextBody({ def, el, screenW }: { def: GameDef; el: VarTextEl; screenW: number }) {
  const v = def.variables.find((x) => x.id === el.varId);
  const sample = v ? String(v.initial) : '⚠';
  return (
    <span className="tt-el-body tt-text-body" style={{ justifyContent: JUSTIFY[el.align] }}>
      <span style={textStyle(el, screenW)}>{`${el.label ?? ''}${sample}`}</span>
    </span>
  );
}
